...
public class Game extends AppCompatActivity implements View.OnClickListener {
    ...
    private void checkWin() {
        ...
        if (emptyX == 3 && emptyY == 3) {
            for (int i = 0; i < bttngroup.getChildCount() - 1; i++) {
        ...
        if (isWin) {
            Toast.makeText(this, "You have completed the puzzle!\nYou win!", Toast.LENGTH_LONG).show();
            for (int i = 0; i < bttngroup.getChildCount() - 1; i++) {
    ...
    private void loadViews() {
        bttngroup = findViewById(R.id.buttongroup);
        buttons = new Button[4][4];


        for (int i = 0; i < bttngroup.getChildCount(); i++) {
            buttons[i/4][i%4] = (Button)bttngroup.getChildAt(i);
        }
    }
